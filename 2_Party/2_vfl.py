...
class TopModel(nn.Module):
    ...
    def __init__(self, input_dim=64, num_classes=11):  # 32*2 clients
        super().__init__()
        self.net = nn.Sequential(
...
def train_one_epoch(client1_loader, client2_loader, 
    ...
    for (x1, y), (x2,) in zip(client1_loader, client2_loader):
        ...
        # Forward pass (federated)
        with torch.no_grad():  # Other clients compute without gradients
            h2 = client2_bottom(x2)
        h1 = client1_bottom(x1)
        h_combined = torch.cat([h1, h2], dim=1)
        outputs = top_model(h_combined)
        
        # Backward pass (only active client updates)
        optimizer.zero_grad()
...
def main():
    ...
    # Only active client's bottom model and top model have trainable parameters
    optimizer = optim.Adam(
        list(client1_bottom.parameters()) + list(top_model.parameters()), 
