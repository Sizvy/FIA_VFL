...
def save_client_data(X, y, prefix, split_type='train'):
    ...
    if prefix == 'shadow_plus_F':
        ...
        y, _ = np.split(y, 2)
        np.save(f'{OUTPUT_DIR}/{prefix}_client_2_{split_type}_inter.npy', client2_data)
    else:
    ...
    np.save(f'{OUTPUT_DIR}/{prefix}_client_1_{split_type}_inter.npy', client1_data)
    np.save(f'{OUTPUT_DIR}/{prefix}_client_1_{split_type}_labels_inter.npy', y)
...
if __name__ == "__main__":
    ...
    - D+F (with F): 
      * Client 1: {OUTPUT_DIR}/shadow_plus_F_client_1_train_inter.npy
      * Client 2: {OUTPUT_DIR}/shadow_plus_F_client_2_train_inter.npy
    - D-F (without F): 
      * Client 1: {OUTPUT_DIR}/shadow_minus_F_client_1_train_inter.npy
      * Client 2: {OUTPUT_DIR}/shadow_minus_F_client_2_train_inter.npy
    """)
