...
def load_shadow_data_inter(prefix):
    X_client1 = np.load(f"{prefix}_client_1_train_inter.npy")
    y = np.load(f"{prefix}_client_1_train_labels_inter.npy")
    
    X_client2 = np.load(f"{prefix}_client_2_train_inter.npy")
...
if __name__ == "__main__":
    ...
    print("\nShadow models saved for intra  embedding analysis.")




    X1_plus_F, X2_plus_F, y_plus_F = load_shadow_data_inter(SHADOW_PLUS_F_PATH)
    X1_minus_F, X2_minus_F, y_minus_F = load_shadow_data_inter(SHADOW_MINUS_F_PATH)

    print("Training shadow model with F...")
    shadow_client2_plus_F_inter, shadow_client1_plus_F_inter = train_shadow_model(X1_plus_F, X2_plus_F, y_plus_F, client2_has_F=True)

    print("\nTraining shadow model without F...")
    shadow_client2_minus_F_inter, shadow_client1_minus_F_inter = train_shadow_model(X1_minus_F, X2_minus_F, y_minus_F, client2_has_F=False)

    torch.save(shadow_client2_plus_F_inter.state_dict(), f"{SHADOW_PLUS_F_PATH}_client2_bottom_inter.pt")
    torch.save(shadow_client2_minus_F_inter.state_dict(), f"{SHADOW_MINUS_F_PATH}_client2_bottom_inter.pt")

    torch.save(shadow_client1_plus_F_inter.state_dict(), f"{SHADOW_PLUS_F_PATH}_client1_bottom_inter.pt")
    torch.save(shadow_client1_minus_F_inter.state_dict(), f"{SHADOW_MINUS_F_PATH}_client1_bottom_inter.pt")
    print("\nShadow models saved for inter embedding analysis.")
