...
def load_and_merge_client_data():
    """
    Loads the split client data from .npy files and merges features from both clients.
    Returns merged training, validation, and test sets along with their labels.
    """
    X_train_parts, X_val_parts, X_test_parts = [], [], []
    
    # Load data from both clients
    for client_id in [1, 2]:
        # Load and append each client's data
        X_train_parts.append(np.load(f'splitted_data/client_{client_id}_train.npy'))
        X_val_parts.append(np.load(f'splitted_data/client_{client_id}_val.npy'))
        X_test_parts.append(np.load(f'splitted_data/client_{client_id}_test.npy'))
    
    # Load labels (from client 1) and ensure they're in range 0-10
    ...
    # Horizontally stack features from both clients
...
class CentralModel(nn.Module):
    """Neural network model for multi-class classification with 11 classes"""
    def __init__(self, input_dim=48, hidden_dim=64, num_classes=11):
...
def save_results(metrics, filename='Results/merged_client_results_all_features.csv'):
...
def main():
    ...
    # 1. Load and merge client data
    X_train, X_val, X_test, y_train, y_val, y_test = load_and_merge_client_data()
    ...
    # 3. Initialize model
    model = CentralModel(input_dim=X_train.shape[1], num_classes=11)
    
    # 4. Train model
    trained_model, history = train_model(model, train_loader, val_loader)
    
    # 5. Load best model and evaluate on test set
    trained_model.load_state_dict(torch.load('Models/merged_client_model_all_features.pt'))
