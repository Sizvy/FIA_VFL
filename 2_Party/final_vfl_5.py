...
class BottomModel(nn.Module):
    """Simple Bottom model (kept minimal)"""
    def __init__(self, input_dim=24, output_dim=32):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, 64),  # Simple projection
            nn.ReLU(),
            nn.Linear(64, output_dim),  # Final embedding
            nn.Tanh()  # Keeping this for bounded outputs
        )
...
class TopModel(nn.Module):
    """Complex Top model (now contains most of the complexity)"""
    def __init__(self, input_dim=64, num_classes=11):  # input_dim = 2*output_dim of BottomModel
        super().__init__()
        self.net = nn.Sequential(
