...
def train_one_epoch(client1_loader, client2_loader, 
    ...
    """Training with DP noise in bottom models"""
    ...
    for (x1, y), (x2,) in zip(client1_loader, client2_loader):
        ...
        # Forward pass with DP noise
        h1 = client1_bottom(x1)  # Already contains noise during training
        h2 = client2_bottom(x2)  # Same here
...
def validate(client1_loader, client2_loader, models, criterion, device):
    ...
    with torch.no_grad():
        for (x1, y), (x2,) in zip(client1_loader, client2_loader):
            ...
            h1 = client1_bottom(x1)  # No noise in eval mode
...
def main():
    ...
    num_epochs = 50
    patience = 5
    dp_noise_scale = 0.5  # Adjust this for privacy/accuracy tradeoff
    ...
    # Initialize models with DP noise
    client1_bottom = BottomModel(
        input_dim=client1_train.shape[1], 
        noise_scale=dp_noise_scale
    ).to(device)
    client2_bottom = BottomModel(
        input_dim=client2_train.shape[1],
        noise_scale=dp_noise_scale
    ).to(device)
    top_model = TopModel().to(device)
    
    # Single optimizer (no DPSGD needed)
